...
class LoanRepaymentSchedule(Document):
	...
	def make_customer_repayment_schedule(self):
		...
		(
			...
			additional_principal_amount,
			pending_prev_days,
		) = self.handle_special_emi_schedule(
				"repayment_schedule",
				previous_interest_amount,
				balance_amount,
				additional_principal_amount,
	...
	def handle_special_emi_schedule(
		self,
		schedule_field,
		previous_interest_amount,
		balance_amount,
		...
		if not self.restructure_type and self.repayment_method != "Repay Fixed Amount per Period":
			...
			standard_monthly_repayment_amount = self.monthly_repayment_amount
		...
		special_emi_count = 0
		if completed_tenure > special_emi_period:
			special_emi_count = special_emi_period
		else:
			special_emi_count = completed_tenure

		while balance_amount > 0 and special_emi_count < special_emi_period:
			if self.moratorium_tenure and self.repayment_frequency == "Monthly":
				if getdate(payment_date) > getdate(self.moratorium_end_date):
					if (
						self.moratorium_type == "EMI"
