    ...
    for entry in interest_accruals:
        ...
        if due_date and getdate(entry.posting_date) < getdate(due_date):
            accrual_map[entry.loan]["interest_outstanding"] += (
                entry.interest_amount - entry.additional_interest_amount
            )
        else:
            accrual_map[entry.loan]["undue_interest"] += entry.interest_amount - entry.additional_interest_amount
        ...
        if last_accrual_date and getdate(entry.posting_date) == last_accrual_date:
            accrual_map[entry.loan]["penalty"] = entry.interest_amount - entry.additional_interest_amount
